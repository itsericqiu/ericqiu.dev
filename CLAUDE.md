...
- Exports: `light`, `dark` (type `PaletteMode`), `icon` (`{ bg, text }`), `accentCycle` (type `CycleColors`)
...
- Icon/favicon: **amber** `#5A3E14` bg (navy `#1B2D45` was tried, reverted Mar 2026)
- Dual-accent structure: `--accent` (structural) + `--accent-2` (editorial — kicker, dates); currently equal for amber palette
  - `PaletteMode` fields: `accent`, `accentLight`, `accentDark`, `accentSecondary`, `accentSecondaryLight`, `accentSecondaryDark`
  - CSS vars: `--accent`, `--accent-light`, `--accent-dark`, `--accent-2`, `--accent-2-light`, `--accent-2-dark`
- `accentCycle` — `{ light: string[], dark: string[] }` — hero letter-seg hues; injected into Hero.astro via `define:vars`
...
- **Letter-seg color cycling** (`Hero.astro`): JS state machine cycles "Eric" through accent hues every 4s. Uses diagonal scan wipe: `@property --scan-pos` (`<percentage>`) animates gradient boundary in `@keyframes scan-wipe` at 55°, with `background-clip: text` on `.hero-line:first-child.is-scanning`. **Key detail**: scan animation driven via `el.style.animation` (inline style) — NOT from CSS class — to prevent the CSS cascade from replaying `line-reveal` when `.is-scanning` is removed. `line-reveal` is frozen via an `animationend` listener after page-load entry. Cycle colors derived from `accentCycle` in `palette.ts` via `define:vars` — light: 4 hues, dark: 6 warm-calibrated hues. Theme-switch snapped via MutationObserver on `data-theme`. Cycle length is `palette().length` (dynamic). First transition at 4s, then every 4s. No kicker pulse.
...
src/
  ...
  palette.ts               — all color tokens: light, dark, icon, accentCycle exports
  assets/
...
public/
  favicon.svg              — vector EQ lettermark (served first for modern browsers)
  favicon.png              — 256×256 EQ lettermark PNG fallback (?v=2 cache-bust)
  icon-1024.png            — 1024×1024 EQ lettermark (high-DPI PWA)
  icon-512.png             — 512×512 EQ lettermark
  icon-192.png             — 192×192 EQ lettermark
  apple-touch-icon.png     — 180×180; legacy iOS variants also present
  profile.jpg              — profile photo (footer)
  _lab.html                — internal design lab index (NOT part of Astro build)
  palette-preview.html     — palette explorer: 6 standard + 8 bold + 16 additive signature palettes
...
- `_lab-palettes.js` — single source of truth for all 62 palette entries (34 standard/signature/bold + 28 editorial), `paletteOptions` (for `<select>` menus), and named presets. Each entry has: `bg`, `bg2`, `text`, `text2`, `muted`, `accent`, `accentLight`, `accentDark`, `accent2`, `accent2Light`, `accent2Dark`, `border`, `borderS`, `nav`
